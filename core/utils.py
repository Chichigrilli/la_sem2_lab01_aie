...
def validate_shape(
    ...
    if not isinstance(shape, (tuple, list)):
        raise TypeError(f"shape должна быть tuple или list, получено {type(shape)}")

    shape_tuple = tuple(shape)
    for i, dim in enumerate(shape_tuple):
        if not isinstance(dim, int) or dim <= 0:
            raise ValueError(f"Размер по моде {i} должен быть положительным целым числом, получено {dim}")
...
def flat_to_multi_index(
    ...
    for i in range(len(shape) - 1, -1, -1):
        multi_index.append(remaining % shape[i])
        remaining //= shape[i]
